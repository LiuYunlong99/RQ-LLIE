...
def tensor2img(tensor, out_type=np.uint8, min_max=(0, 1)):
    ...
    # img_np = img_np * 1.2
    # img_np = np.clip(img_np, a_min=0,a_max=255.0)
    return img_np.astype(out_type)


def feature2img(tensor, tensor1, out_type=np.uint8, min_max=(0, 1)):
    temp = torch.cat([tensor.view(tensor.shape[0], -1), tensor1.view(tensor.shape[0], -1)], dim=-1)
    temp_min = temp.min(dim=-1, keepdim=True).values.unsqueeze(-1)
    temp_max = temp.max(dim=-1, keepdim=True).values.unsqueeze(-1)
    tensor = (tensor - temp_min) / (temp_max - temp_min)
    tensor1 = (tensor1 - temp_min) / (temp_max - temp_min)
    ...
    return to_img(tensor), to_img(tensor1)
