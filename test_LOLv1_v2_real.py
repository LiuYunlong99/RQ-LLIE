...
def main():
    ...
    if opt['stage'] == 2:
        mask_folder = osp.join(save_folder, 'images/mask')
        ...
        for val_data in val_loader:
            ...
            if save_imgs:
                try:
                    ...
                    # if opt['stage'] == 2:
                    #     io.savemat(osp.join(mask_folder, '{}.mat'.format(tag)), {'mask': visuals['mask'].numpy()})
                        # cv2.imwrite(osp.join(mask_folder, '{}.png'.format(tag)), mask_img)
                        # plt.savefig(osp.join(mask_folder, '{}.png'.format(tag)))
